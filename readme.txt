# CMS BDU - Hệ thống Quản lý lớp học và hỗ trợ học vụ

Hệ thống Quản lý lớp học và hỗ trợ học vụ (class Management System) dành cho khoa CNTT Trường Đại học Bình Dương (BDU).
...
### 1. Cài đặt XAMPP và chuẩn bị mã nguồn

1. [Tải và cài đặt XAMPP](https://www.apachefriends.org/index.html) (chọn phiên bản PHP >= 8.0).
2. Mở XAMPP Control Panel, **Start** Apache và MySQL.
3. Tải mã nguồn về thư mục web:
       - Nếu dùng Git:
         ```bash
         git clone https://github.com/Nhat-IT/cms-bdu-web.git
         ```
       - Hoặc giải nén file zip vào thư mục: `C:/xampp/htdocs/cms`

### 2. Tạo database và import dữ liệu

1. Truy cập [http://localhost/phpmyadmin](http://localhost/phpmyadmin)
2. Tạo database mới tên `cms_bdu` với charset `utf8mb4_unicode_ci`.
3. Import file `db.sql` (nằm trong thư mục dự án) vào database vừa tạo.
       - Trong phpMyAdmin: Chọn database `cms_bdu` → tab **Import** → chọn file `db.sql` → **Go**.
       

### 3. Cấu hình kết nối database

1. Copy file `.env.example` thành `.env` trong thư mục gốc dự án:
       ```bash
       cp .env.example .env
       ```
       (Hoặc tạo mới file `.env` nếu không có lệnh `cp` trên Windows)
2. Mở file `.env`, chỉnh sửa thông tin kết nối:
       ```env
       DB_HOST=localhost
       DB_USER=root
       ...
       DB_PORT=3306
       ```
       - Thay `your_password` bằng mật khẩu MySQL của bạn (mặc định XAMPP thường để trống).

### 4. Chạy hệ thống trên XAMPP

1. Đảm bảo Apache và MySQL đã **Start** trong XAMPP Control Panel.
2. Truy cập hệ thống qua trình duyệt:
       - [http://localhost/cms/](http://localhost/cms/)
