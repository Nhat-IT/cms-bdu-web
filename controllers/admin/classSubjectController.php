...
POST['group_code'])) {
            $groupData = db_fetch_one('SELECT id FROM class_subject_groups WHERE class_subject_id = ? AND group_code = ? LIMIT 1', [<?php
/**
 * CMS BDU - Class Subject Controller
 * Xử lý thao tác đóng/mở lớp học phần và cập nhật lịch nhóm.
 */
...
POST['extra_is_regular']) ? 1 : 0;
        ...
        if ($existingExtra) {
            ...
            logSystem("Cap nhat buoi hoc theo ngay (ID #$extraId) cho nhom ID #$groupId", 'extra_classes', $extraId);
        } else {
            db_query(
                'INSERT INTO extra_classes (class_subject_group_id, extra_date, day_of_week, start_period, end_period, room, note, is_regular)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
            ...
            logSystem("Them buoi hoc bo sung cho nhom ID #$groupId", 'extra_classes', $extraId > 0 ? $extraId : null);
        }
...
POST['format'] === 'json') {
            jsonResponse([
                'ok' => true,
                'message' => $existingExtra ? 'extra_class_updated' : 'extra_class_added',
